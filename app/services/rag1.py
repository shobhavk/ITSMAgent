...
class TicketIndex:
    ...
    the search_similar_tickets tool, the no-LLM fallback search, and
    recurring_issues.py's semantic clustering (cluster_similar)."""
    ...
    async def cluster_similar(self, similarity_cutoff: float = 0.85, min_cluster_size: int = 3) -> list[list[int]]:
        ...
        O(n^2) in both time and memory (a full similarity matrix) - fine
        for the hundreds-to-few-thousand tickets a single analysis batch
        typically has; recurring_issues.py caps the batch size before
        calling this so it never runs on an unbounded n.

        Falls back to exact-normalized-text grouping (_keyword_cluster)
        if no embeddings model is configured or embedding fails - reduced
        recall (won't catch reworded duplicates) but still functional in
        rule_based mode, consistent with search()'s fallback above."""
        n = len(self.rows)
        if n == 0:
            return []
    ...
    def _keyword_cluster(self, min_cluster_size: int) -> list[list[int]]:
        ...
        the exact same value (lowercased, whitespace-collapsed). Only
        catches near-identical wording, not paraphrased recurrences -
        that's the tradeoff for not needing an embeddings model."""
